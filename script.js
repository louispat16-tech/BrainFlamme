        ...
        quizHistory.forEach((item, index) => { 
            ...
            recapHTML += `
                <div style="background:#1e293b; padding:15px; border-radius:15px; margin-bottom:15px; border-left:6px solid ${color};"> 
                    ...
                    <p style="color:${color}; font-weight:bold; margin:0; font-size:15px;"> ${icon} Ta réponse : ${item.userAns} </p> 
                    ${!item.isCorrect ? `<p style="color:#94a3b8; margin:5px 0 0 0; font-size:14px;">La réponse était : ${item.correctAns}</p>` : ''} 
                </div>
            `; 
        });
    }
